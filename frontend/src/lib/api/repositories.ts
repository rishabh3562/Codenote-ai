...
export const repositoryApi = {
  ...
  create: async (data: Partial<Repository>) => {
    const response = await apiClient.post<Repository>(API_ENDPOINTS.repositories.create, data);
    return response.data;
  },

  update: async (id: string, data: Partial<Repository>) => {
    const response = await apiClient.put<Repository>(API_ENDPOINTS.repositories.update(id), data);
    return response.data;
  },
